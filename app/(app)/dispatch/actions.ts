...
export async function dispatchAction(input: unknown): Promise<DispatchState> {
  ...
  // same narrowed-not-closed defence the assign screen's own pre-write check
  // gives it — so dispatching a request someone else already moved fails
  // loudly instead of silently re-dispatching it.
  ...
  // defence the assign screen's own pre-write check gives it. `Assigned` is
  // what every one of these tools should read at this point — see
  // `isReadyForDispatch`'s doc comment.
