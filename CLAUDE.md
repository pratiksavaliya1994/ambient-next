...
**`/tools/[toolId]` is the one screen that edits a tool directly, and the only `tools` writer in this app that is not a Bubble workflow** — it calls `bubblePatch` (`lib/bubble/tool-detail.ts`), which had no caller before it. That is deliberate: every `/wf/` write exists because it does more than one thing (fans out over a list, creates child rows, sets `request.status` in the same breath), and editing one row's fields does none of that. **Two things to know before changing it.** First, the gate on `location`/`floor`/`currentUser`/`statusNew` is **claim-based, not status-based** — `readToolHold` asks `listToolClaims` and `listRequestClaims` whether an open trip or open request holds the tool, because `statusNew` lies in both directions: a **refused** tool reads `Available` at `"Warehouse"` while physically still on the truck, and an **orphaned** tool reads `Assigned` with no row behind it and has silently dropped out of every picker. Freeing those orphans is what the page is for; the reasoning is on `lib/tools/tool-edit.ts`. Second, **`Available` is the only `statusNew` value it can write** (`ToolPatch` types the field to that literal) — every other value asserts a claim that needs an `assignedtools` or `triptool` row behind it, and this page creates neither; a broken or lost tool is recorded through `condition` instead. **Unverified as of 2026-09-17:** `DB - Tools Change Log` is expected to fire on a Data API `PATCH` the same as on a workflow edit, but no `tools` write has ever reached Bubble that way from this app — confirm a `toolshistory` row appears after the first real save. If it doesn't, the fallback is a thin `/wf/update-tool` and `updateTool` is the only function that changes.

The Pickup flow's per-tool `statusNew` write-back is fanned out inside Bubble from `new-pickup-request` via `update-tool-status` (originally built against the old `status` field — see `bubble-pickup-tool-status-workflow.md` — and since repointed at `statusNew` on the Bubble side). Phase 2 additionally writes `location` on dispatch and offload, via `update-request-status`; see [`docs/phase-2-lifecycle.md`](docs/phase-2-lifecycle.md). **Dispatch writes a _subset_ of a request's tools, not all of them** — a tool sitting `Available` on another job site is deliberately left out of that call so Bubble leaves it where it is, and moves only when a driver confirms collecting it (`confirmPickupAction`); `isWarehouseLocation` (`lib/bubble/enums.ts`) is what decides which side a tool falls on. See [`docs/phase-2d-site-to-site-transfers.md`](docs/phase-2d-site-to-site-transfers.md). Note the old Bubble UI is a **second writer** of `location` (and of the old `status` field) via the pre-existing `/wf/Set Status` and `/wf/Set Location` — and, like every other writer of `tools`, its edits also flow into `toolshistory` via `DB - Tools Change Log` (~1,542 pre-existing rows, `prevStatus`/`newStatus` typed to the old `Tool Status` option set). That data-event workflow logs **every** `tools` save automatically, so neither `update-request-status` nor `update-tool-status` log history themselves — they just edit `tools` fields and let `DB - Tools Change Log` do the rest, including populating two lifecycle-typed fields (`prevStatusNew`/`newStatusNew`, option set `ToolStatusNew`) added specifically so it wouldn't have to squeeze `ToolStatusNew` values into the old `Tool Status`-typed `prevStatus`/`newStatus`. An earlier design had `update-request-status` write its own explicit `toolshistory` row per tool; that duplicated `DB - Tools Change Log`'s automatic one and was removed 2026-09-10 (see `docs/bubble-update-request-status-spec.md`).
